...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		disabledNow := service.ProcessChannelError(c, *types.NewChannelError(channel.Id, channel.Type, channel.Name, channel.ChannelInfo.IsMultiKey, common.GetContextKeyString(c, constant.ContextKeyChannelKey), channel.GetAutoBan()), newAPIError, &channelSettings, true, relayInfo)
		if disabledNow {
		...
		if !service.ShouldRetryRelayError(c, newAPIError, common.RetryTimes-retryParam.GetRetry(), &channelSettings) {
			break
		}
	}
...
func executeTaskSubmissionWith(
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		if !taskErr.LocalError {
			channelSettings := channel.GetOtherSettings()
			disabledNow = service.ProcessChannelError(c,
