...
func TestShouldRetryRelayErrorHonorsChannelPinOnChannelError(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, test.wantRetry, ShouldRetryRelayError(c, err, 1, nil))
		})
	}
}
...
func TestProcessChannelErrorMasksDisableReasonAndNotification(t *testing.T) {
	...
	previousRuntimeDisable, previousErrorLog := common.RuntimeAutomaticDisableChannelEnabled, constant.ErrorLogEnabled
	previousRanges := operation_setting.RuntimeAutomaticDisableStatusCodeRanges
	previousKeywords := operation_setting.RuntimeAutomaticDisableKeywords
	...
	t.Cleanup(func() {
		...
		common.RuntimeAutomaticDisableChannelEnabled, constant.ErrorLogEnabled = previousRuntimeDisable, previousErrorLog
		operation_setting.RuntimeAutomaticDisableStatusCodeRanges = previousRanges
		operation_setting.RuntimeAutomaticDisableKeywords = previousKeywords
	...
	common.RuntimeAutomaticDisableChannelEnabled, constant.ErrorLogEnabled = true, false
	operation_setting.RuntimeAutomaticDisableStatusCodeRanges = []operation_setting.StatusCodeRange{{Start: 401, End: 401}}
	operation_setting.RuntimeAutomaticDisableKeywords = nil
	...
	ProcessChannelError(c, types.ChannelError{ChannelId: channel.Id, ChannelName: channel.Name, AutoBan: true}, apiErr, nil, true, nil)
...
func TestDecideRelayRetryReasons(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.want.Action == "retry", ShouldRetryRelayError(c, tc.err, tc.retries, nil))
		})
	}
}
...
func TestRequestPolicyEventsReachLogAdminInfo(t *testing.T) {
	...
	// The fork's ShouldDisableChannel combines status ranges and keywords with AND
	// semantics, so pin an explicit status-only rule here; otherwise the default
	// keyword list would gate the 401 "invalid credential" failure out of the
	// disable decision and the health entry would read "unchanged".
