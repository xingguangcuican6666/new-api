	...
	if s.lockedChannelID != 0 {
		...
		for ; retry.GetRetry() <= common.RetryTimes; retry.IncreaseRetry() {
			...
			if apiErr != nil {
				...
				service.ProcessChannelError(c, *types.NewChannelError(channel.Id, channel.Type, channel.Name, channel.ChannelInfo.IsMultiKey, info.ApiKey, channel.GetAutoBan()), apiErr, &channelSettings, true, info)
				if service.ShouldRetryRelayError(c, apiErr, common.RetryTimes-retry.GetRetry(), &channelSettings) {
					continue
				}
				return apiErr
			}
