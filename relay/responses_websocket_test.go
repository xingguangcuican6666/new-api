...
func TestSelectResponsesWSChannelHonorsPinsAndFilters(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.False(t, service.ShouldRetryRelayError(c, types.NewErrorWithStatusCode(errors.New("upstream failed"), types.ErrorCodeDoRequestFailed, 503), 2, nil))
		})
	}
}
...
func TestResponsesWSChannelRoutingRequiresExplicitOptIn(t *testing.T) {
	...
	require.NotNil(t, channel)
	assert.Equal(t, enabled.Id, channel.Id)
	httpChannel, err := model.GetRandomSatisfiedChannel([]string{"default"}, "ws-model", 0, nil)
	require.NoError(t, err)
