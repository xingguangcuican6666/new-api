...
// single reverse proxy (e.g. nginx) where all clients share one address, and
// DisableRateLimit turns request-rate limiting off entirely. Here the anonymous
// invalid-token budget is IP-keyed, so either switch drops it; the global and
// per-object concurrency budgets stay active regardless.
