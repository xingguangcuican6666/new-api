...
export function ChannelMutateDrawer({
  ...
  const currentBillingQuery = formValues.billing_query
    ...
    >
        ...
        render={({ field }) => (
          <FormItem className='flex items-center justify-between sm:col-span-2'>
            <div className='space-y-0.5'>
              ...
              <FormDescription>
                {t(
                  'Retry matching upstream errors before returning an error to the client'
                )}
              </FormDescription>
            </div>
            <FormControl>
      ...
      {form.watch('automatic_retry_override_enabled') && (
        <>
          <FormField
            control={form.control}
            name='automatic_retry_status_codes'
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Forced retry status codes')}</FormLabel>
                <FormControl>
          ...
          <FormField
            control={form.control}
            name='automatic_retry_keywords'
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Forced retry keywords')}</FormLabel>
                <FormControl>
                  <Textarea
                    rows={4}
                    placeholder={t('one keyword per line')}
                ...
                <FormDescription>
                  {t(
                    'When both status codes and keywords are set, both must match'
                  )}
                </FormDescription>
                <FormMessage />
              </FormItem>
  ...
  const ratioProbeFields = (
    <div
      className={channelConfigurationBlockClassName(
        configuration.blocks.ratioProbe,
        'space-y-4 scroll-mt-4'
      )}
    >
      <CardHeading
        status={configuration.blocks.ratioProbe}
      ...
      >
        <div className='divide-border space-y-0 divide-y border-y'>
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex items-center justify-between px-4 py-3'>
                <div className='space-y-0.5'>
                  <FormLabel>{t('Enable ratio probe')}</FormLabel>
                  <FormDescription>
                    {t(
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex items-center justify-between px-4 py-3'>
                <div className='space-y-0.5'>
                  <FormLabel>{t('Probe with the channel API key')}</FormLabel>
                  <FormDescription>{ratioProbeAuthDescription}</FormDescription>
                </div>
                <FormControl>
                  <Switch
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('Authorization header (optional)')}</FormLabel>
              <FormControl>
                <Input
                  type='password'
                  autoComplete='off'
                  placeholder={t('Bearer token or other Authorization value')}
                  disabled={!ratioProbeEnabled || ratioProbeUsesAPIKey}
                  {...field}
                />
              </FormControl>
              <FormDescription>
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select
                ...
                value={field.value || RATIO_PROBE_SOURCE_FOLLOW_API}
                disabled={!ratioProbeEnabled}
              >
                <FormControl>
                  <SelectTrigger>
                ...
                <SelectContent alignItemWithTrigger={false}>
                  <SelectGroup>
                    <SelectItem value={RATIO_PROBE_SOURCE_FOLLOW_API}>
                      {t('Follow the API Base URL')}
                    </SelectItem>
                    <SelectItem value={RATIO_PROBE_SOURCE_CUSTOM}>
                      {t('Custom Base URL')}
                    </SelectItem>
                  </SelectGroup>
                </SelectContent>
        ...
        {ratioProbeUsesCustomSource && (
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Ratio probe Base URL')}</FormLabel>
                <FormControl>
                  <Input
                    placeholder={t('e.g., https://new-api.example.com')}
                    disabled={!ratioProbeEnabled}
                    {...field}
                  />
                </FormControl>
              ...
              <FormDescription>
                {ratioProbeRequestURL ? (
                  <span className='block font-mono text-xs'>
                    {t('Request')}: GET {ratioProbeRequestURL}
                  </span>
                ) : (
                  t('Set a Base URL to show the request path.')
                )}
              </FormDescription>
              <FormMessage />
            </FormItem>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('Upstream group to inspect')}</FormLabel>
              <FormControl>
                <Input
        ...
        <div className='grid gap-4 sm:grid-cols-2'>
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Max group ratio')}</FormLabel>
                <FormControl>
                  <Input
                    inputMode='decimal'
                    placeholder='1'
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Min group ratio')}</FormLabel>
                <FormControl>
                  <Input
                    inputMode='decimal'
        ...
        {ratioProbeMeta && (
          <div className='text-muted-foreground space-y-2 border-t pt-3 text-xs'>
            ...
            <div className='flex flex-wrap items-center gap-2'>
              ...
              <Badge
                variant={
                  ratioProbeMeta.lastStatus === RATIO_PROBE_STATUS_COMPLIANT
                    ? 'secondary'
                    : 'destructive'
                }
              >
                {t(
                  RATIO_PROBE_STATUS_LABELS[ratioProbeMeta.lastStatus] ||
                    ratioProbeMeta.lastStatus
                )}
              </Badge>
              {ratioProbeMeta.lastGroupRatio !== null && (
                <Badge variant='outline'>
                  {t('Group ratio')}: {ratioProbeMeta.lastGroupRatio}
                </Badge>
              )}
              {ratioProbeMeta.lastDisabledKeys > 0 && (
                <Badge variant='outline'>
                  {t('Disabled keys')}: {ratioProbeMeta.lastDisabledKeys}
                </Badge>
              )}
              {ratioProbeMeta.lastEnabledKeys > 0 && (
                <Badge variant='outline'>
                  {t('Restored keys')}: {ratioProbeMeta.lastEnabledKeys}
                </Badge>
              )}
            </div>
            {ratioProbeMeta.lastMessage && (
              <div className='break-all'>{ratioProbeMeta.lastMessage}</div>
            )}
          </div>
        )}
      </fieldset>
      ...
      <ChannelApiAccessSection>
        ...
        <div className='border-border/60 flex flex-col gap-3 border-t pt-4'>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Balance query type')}</FormLabel>
                <Select
                  items={[
                    {
                      value: BILLING_QUERY_DEFAULT_OPTION,
                      label: t('Use channel default balance query'),
                    },
                    {
                  ...
                  onValueChange={(value) => {
                    field.onChange(
                      value === BILLING_QUERY_DEFAULT_OPTION ? '' : value
                    )
                  }}
                  value={field.value || BILLING_QUERY_DEFAULT_OPTION}
                >
                  <FormControl>
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t('Select a balance query type')}
                      />
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent alignItemWithTrigger={false}>
                    <SelectGroup>
                      <SelectItem value={BILLING_QUERY_DEFAULT_OPTION}>
                        {t('Use channel default balance query')}
                      </SelectItem>
                      <SelectItem value={BILLING_QUERY_TYPE_NEW_API}>
                        {t('New API')}
                      </SelectItem>
                    </SelectGroup>
                  </SelectContent>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>
                  {t('Sent as the New-Api-User header when provided.')}
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Balance query endpoint')}</FormLabel>
                <FormControl>
                  <Input
                    placeholder={t('e.g., https://new-api.example.com')}
                    disabled={!billingQueryType}
                ...
                <FormDescription>
                  {billingQueryRequestURL ? (
                    <>
                      ...
                      <span className='block font-mono text-xs'>
                        {t('Request')}: GET {billingQueryRequestURL}
                      </span>
                    </>
                  ) : (
                    t(
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex items-center justify-between gap-4'>
                <div className='space-y-0.5'>
                  <FormLabel>{t('Use channel API key')}</FormLabel>
                  <FormDescription>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Custom Bearer token')}</FormLabel>
                <FormControl>
                  <Input
                    type='password'
                    autoComplete='off'
                    placeholder={t('Enter an independent Bearer token')}
                    disabled={!billingQueryType || billingQueryUsesAPIKey}
                    {...field}
                  />
                </FormControl>
                <FormDescription>
