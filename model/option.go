...
func InitOptionMap() {
	...
	common.OptionMap["UpstreamPrivacyProtectionEnabled"] = strconv.FormatBool(setting.UpstreamPrivacyProtectionEnabled)
	...
	common.OptionMap[setting.DefaultRegistrationGroupOptionKey] = setting.DefaultRegistrationGroup
	...
	common.OptionMap["NginxMode"] = strconv.FormatBool(common.NginxMode)
	common.OptionMap["DisableRateLimit"] = strconv.FormatBool(common.DisableRateLimit)
...
func UpdateOptionsBulk(values map[string]string) error {
	...
	for key := range values {
		if isRegistrationGroupOption(key) {
			return updateRegistrationGroupOptions(values)
		}
	}
...
func updateOptionMap(key string, value string) (err error) {
	if isRegistrationGroupOption(key) {
		return fmt.Errorf("registration group options must be applied together")
	}
	...
	if strings.HasSuffix(key, "Enabled") || key == "DefaultCollapseSidebar" || key == "DefaultUseAutoGroup" || key == "SMTPForceAuthLogin" || key == "SMTPInsecureSkipVerify" || key == "NginxMode" || key == "DisableRateLimit" {
		...
		case "UpstreamPrivacyProtectionEnabled":
			setting.UpstreamPrivacyProtectionEnabled = boolValue
		...
		case "NginxMode":
			common.NginxMode = boolValue
		case "DisableRateLimit":
			common.DisableRateLimit = boolValue
	...
	case "ChannelCooldownBaseSeconds":
		common.ChannelCooldownBaseSeconds, _ = strconv.Atoi(value)
